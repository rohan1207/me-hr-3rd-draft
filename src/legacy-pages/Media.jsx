...
function InsightCard({ post, large = false, reduce, compact = false }) {
  return (
    <Link
      to={`/media/${post.id}`}
      className={`group flex h-full flex-col overflow-hidden border border-mehr-deep/8 bg-white shadow-soft transition duration-300 hover:-translate-y-1 hover:border-mehr-deep/18 hover:shadow-float ${
        compact
          ? "rounded-[1.05rem]"
    ...
    >
      ...
      <div
        className={`flex flex-1 flex-col ${
          compact ? "p-2.5" : "p-4 sm:p-6"
        } ${large ? "lg:justify-center lg:p-8" : ""}`}
      >
        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
          <span
            className={`rounded-full bg-mehr-panel font-semibold uppercase tracking-[0.12em] text-mehr-deep ${
              compact
        ...
        <h3
          className={`font-sans font-semibold leading-snug tracking-[-0.02em] text-mehr-ink ${
            large
              ? "mt-2.5 text-[clamp(1.2rem,4.5vw,1.85rem)] sm:mt-3"
              : compact
                ? "mt-1.5 line-clamp-2 text-[12px]"
                : "mt-2.5 text-[14px] sm:mt-3 sm:text-base"
          }`}
        >
          {post.title}
        </h3>
        <p
          className={`leading-relaxed text-mehr-mist ${
            compact
              ...
              : `mt-2 text-[12px] sm:text-sm ${large ? "line-clamp-3" : "line-clamp-2"}`
          }`}
        >
          {post.excerpt}
        </p>
        <span
          className={`mt-auto inline-flex items-center gap-1 font-semibold text-mehr-deep transition group-hover:gap-2 ${
            compact
              ? "pt-2 text-[11px]"
              : "gap-1.5 pt-3.5 text-[13px] group-hover:gap-2.5 sm:pt-4 sm:text-sm"
          }`}
        >
          {compact ? "Read" : "Read full insight"}
          <ArrowUpRight size={compact ? 12 : 15} />
        </span>
      </div>
    </Link>
  );
...
export default function Media() {
  ...
  const visiblePhone = expanded ? filtered : filtered.slice(0, PHONE_PREVIEW);
  const hiddenCount = Math.max(0, filtered.length - PHONE_PREVIEW);
        ...
        <div className="container-mehr page-gutter sm:px-3 md:px-4 lg:px-5">
          ...
          {filtered.length > 0 ? (
            <>
              ...
              {/* sm+: full grid */}
              <RevealStagger
                className="mt-8 hidden gap-4 sm:grid sm:grid-cols-2 lg:grid-cols-3 lg:gap-5"
                stagger={0.05}
              >
                {filtered.map((post) => (
