    ...
    <section className="surface-panel relative overflow-hidden px-4 pb-8 pt-5 text-mehr-ink sm:px-3 sm:pb-10 sm:pt-6 md:px-4 lg:px-5 lg:pb-12 lg:pt-8">
      ...
      <div className="container-mehr page-gutter relative z-10 sm:px-3 md:px-4 lg:px-5">
        <div className="grid items-center gap-5 sm:gap-6 lg:grid-cols-[1.2fr_auto] lg:gap-10">
          <Reveal y={16} className="text-center lg:text-left">
            {eyebrow && <p className="eyebrow mb-2.5 sm:mb-3">{eyebrow}</p>}
            <h2 className="mx-auto max-w-[18ch] font-sans text-[clamp(1.45rem,6vw,2.65rem)] font-semibold leading-[1.1] tracking-[-0.03em] text-mehr-ink lg:mx-0">
              {title}
              {subtitle && (
            ...
            {body && (
              <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-mehr-mist sm:text-[15px] lg:mx-0">
                {body}
              </p>
            )}
          </Reveal>

          <Reveal delay={0.08} y={16} className="relative flex items-center justify-center lg:justify-end">
            <div
              aria-hidden
