...
const SCENE = "/payroll-outsourcing-hero.png";
...
function LivePayrollPanel({ reduce }) {
  ...
  return (
    <div className="grid h-full min-h-[420px] grid-cols-2 gap-2.5 sm:min-h-[480px] sm:gap-3 lg:grid-cols-5 lg:grid-rows-[minmax(0,1.2fr)_minmax(0,1fr)_auto]">
      ...
      {/* Payroll run */}
      <motion.div
        className="col-span-2 flex flex-col justify-between rounded-[1.35rem] border border-mehr-deep/10 bg-white p-4 shadow-soft sm:rounded-[1.5rem] sm:p-5 lg:col-span-2"
      ...
      {/* Statutory */}
      <motion.div
        className="col-span-1 flex flex-col justify-between rounded-[1.35rem] border border-mehr-deep/10 bg-mehr-panel p-4 sm:rounded-[1.5rem] lg:col-span-2"
        animate={reduce ? undefined : { y: [0, 3, 0] }}
      ...
      >
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {STATS.map((tag, i) => (
            <motion.span
              key={tag}
              className="rounded-full bg-white px-2.5 py-1 text-[11px] font-semibold text-mehr-deep shadow-soft"
      ...
      {/* Compact cycle on mobile next to statutory; full-width bar on lg */}
      <div className="col-span-1 flex flex-col justify-center rounded-[1.35rem] bg-mehr-deep p-4 text-white sm:rounded-[1.5rem] lg:col-span-5">
        ...
        <AnimatePresence mode="wait">
          <motion.p
            ...
            className="mt-1 font-sans text-base font-semibold tracking-tight sm:text-lg"
          >
            {CYCLE[step]}
          </motion.p>
        </AnimatePresence>
